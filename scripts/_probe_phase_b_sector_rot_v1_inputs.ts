/**
 * Pre-flight probe for the Phase B sector_rot_v1 campaign harness (Cycle 25,
 * Composite worker, ADR-051 + docs/specs/phase-b-sector_rot_v1.md).
 *
 * Per SPEC §1 + §8 watch-out: `quantlab.sector_rotation_snapshots` was
 * forward-only at SPEC-write time (daemon hook fires Cycle 21+, no
 * historical backfill). The campaign's `loadScoreSeries` query reads
 * `defensive_cyclical_spread_z` from that table; an empty/sparse snapshots
 ...
 *
 * Probe scope (4 reads):
 *   1. Required CH tables exist (sector_rotation_snapshots, phase_b_trials,
 *      phase_b_verdicts; Phase B tables seeded by Cycle 23 migrations).
 *   2. sector_rotation_snapshots — row count + min/max snapshot_date +
 *      defensive_cyclical_spread_z null-coverage. Reports ambiguous-state per
 ...
 *      benchmark resolution as cycle_v1/vol_struct_v1 (these were backfilled
 *      in Cycle 23 per S96-117).
 *   4. Sector-ETF candles for the 6 defensives+cyclicals
 *      (XLP/XLU/XLV/XLY/XLK/XLF). These are the only candles
 *      `defensiveCyclicalSpreadZ` depends on (XLC/XLRE not needed for the
 *      selected score; see SPEC §S-PBSR1-5 note on regimeFlag carve-out).
 *
...
export const help: HelpEntry[] = [
  {
    npm: '_probe:phase_b_sector_rot_v1',
    category: 'Data quality',
    what:
      'Pre-flight probe for Phase B sector_rot_v1 campaign — confirms ' +
      'sector_rotation_snapshots + SPY/QQQ/IWM candles + 6 sector-ETF candles ' +
      '(XLP/XLU/XLV/XLY/XLK/XLF) are present with sufficient history. No DDL; ' +
      'exit 1 if any gap.',
  },
];

/** Benchmarks per SPEC §S-PBSR1-3 (identical to cycle_v1/vol_struct_v1). */
const TARGET_BENCHMARKS = ['SPY', 'QQQ', 'IWM'] as const;

/** 6 sector ETFs the selected score (`defensiveCyclicalSpreadZ`) depends on.
 *  XLP/XLU/XLV defensive basket; XLY/XLK/XLF cyclical basket. All six
 *  launched 1998-12-22 so coverage is not a constraint for the 2013 window.
 *  XLC (2018-09-24) and XLRE (2015-10-08) are NOT required for the selected
 *  score per SPEC §S-PBSR1-5. */
const SCORE_SECTOR_ADDRS = [
  'XLP_USD', 'XLU_USD', 'XLV_USD',  // defensives
  'XLY_USD', 'XLK_USD', 'XLF_USD',  // cyclicals
] as const;

/** Pinned per SPEC §S-PBSR1-5: first trading day of 2013 in US;
 *  matches vol_struct_v1 + cycle_v1 alignment for cross-composite parity. */
 ...
 *  trailing-1y baseline could not have been fully populated at start.
 *  Note: sector_rot_v1 uses a 1y baseline (BASELINE_WINDOW_DAYS = 252)
 *  per sector_rotation_repository.ts:50; we still pin 2014-01-01 to
 *  mirror vol_struct_v1's probe (anti-drift across composite probes). */
...
/** Number of trading days the backfill must populate (per SPEC §S-PBSR1-5
...
export interface PreflightResult {
  ok: boolean;
  tables: {
    sectorRotationSnapshotsExists: boolean;
  ...
  scoreSectors: PreflightCandleCoverage[];
  blocker?: string;
...
export async function runPreflight(): Promise<PreflightResult> {
  ...
  const [snapExists, trialsExists, verdictsExists] = await Promise.all([
    tableExists(ch, 'sector_rotation_snapshots'),
    tableExists(ch, 'phase_b_trials'),
    tableExists(ch, 'phase_b_verdicts'),
  ]);

  // ── 2. sector_rotation_snapshots coverage ─────────────────────────────
  ...
  if (!snapExists) {
    snapshots = {
      ...
      state: 'unknown',
      reason:
        'quantlab.sector_rotation_snapshots does NOT exist. Run the ' +
    ...
    if (rowCount === 0) {
      state = 'empty';
      reason =
        'sector_rotation_snapshots is empty (0 rows). Backfill required ' +
      ...
      state = 'empty';
      reason =
        `sector_rotation_snapshots has only ${rowCount} forward-only rows ` +
      ...
      state = 'ambiguous';
      reason =
        `sector_rotation_snapshots has ${rowCount} rows (need ≥${SPARSE_ROW_THRESHOLD}), ` +
    ...
    snapshots = { rowCount, notNullSpreadZ: notNullZ, minDate, maxDate, state, reason };
  }
  ...
  // ── 4. Score-required sector-ETF candles probe ────────────────────────
  // The selected score `defensiveCyclicalSpreadZ` requires XLP/XLU/XLV +
  // XLY/XLK/XLF (all pre-1999 launches; coverage from 2013-01-03 expected).
  const scoreSectors: PreflightCandleCoverage[] = [];
  for (const addr of SCORE_SECTOR_ADDRS) {
    scoreSectors.push(await probeCandleCoverage(ch, addr, REQUIRED_WINDOW_START));
  }

  // ── 5. Aggregate verdict ──────────────────────────────────────────────
  const tablesOk = snapExists && trialsExists && verdictsExists;
  const benchmarksOk = benchmarks.every(b => b.ok);
  const scoreSectorsOk = scoreSectors.every(s => s.ok);
  ...
  const backfillRecommended =
    snapshots.state === 'empty' && tablesOk && scoreSectorsOk;
  let backfillReason: string | undefined;
  if (backfillRecommended) {
    backfillReason =
      'snapshots table empty + all 6 score-required sector-ETF candles present → ' +
      'safe to invoke `scripts/_backfill_sector_rotation_snapshots.ts --apply` ' +
  ...
  } else if (snapshots.state === 'empty' && !scoreSectorsOk) {
    backfillReason =
      'snapshots empty BUT score-required sector-ETF candles are incomplete → ' +
      'backfill would fail loudly. Resolve sector-ETF ingest first.';
  }

  const overallOk = tablesOk && benchmarksOk && scoreSectorsOk && snapshotsOk;
  let blocker: string | undefined;
  if (!overallOk) {
    const parts: string[] = [];
    if (!snapExists) parts.push('sector_rotation_snapshots table missing');
  ...
  return {
    ok: overallOk,
    tables: {
      sectorRotationSnapshotsExists: snapExists,
...
function formatResult(r: PreflightResult): string {
  const lines: string[] = [];
  lines.push('═══ Phase B sector_rot_v1 pre-flight probe ═══');
  lines.push('');
  lines.push('── 1. Required CH tables ──');
  lines.push(`  sector_rotation_snapshots: ${r.tables.sectorRotationSnapshotsExists ? 'OK' : 'MISSING'}`);
  ...
  lines.push('');
  lines.push('── 2. sector_rotation_snapshots ──');
  lines.push(`  rows:                ${r.snapshots.rowCount}`);
  lines.push(`  not_null spread_z:   ${r.snapshots.notNullSpreadZ}`);
  ...
  lines.push('');
  lines.push('── 4. Score-required sector-ETF candles (interval=1d) ──');
  for (const s of r.scoreSectors) {
 ...
 *   - Convention drift in sector ETF token_addresses (e.g. XLP → XLP_USD_yfinance).
 *     The probe's per-sector coverage check would fail loudly because
 *     the new address would have 0 rows under the old pattern.
 *   - sector_rotation_snapshots schema drift: the probe reads
 *     `defensive_cyclical_spread_z` directly; a column rename would fail
 *     loudly at the SELECT.
