/**
 * Tests for scripts/migrate_create_executive_departure_snapshots.ts (Phase A3).
 *
 * SPEC: docs/specs/executive-departure-signal.md §6.
 *
 * Mirrors the migrateCreateShortInterestSnapshots test pattern byte-for-byte
 * where applicable (FakeClickHouse router + EXPLAIN PLAN grammar check,
 * optional when CH unreachable).
 */
...
import {
  ...
  runPreChecks,
  runPostChecks,
} from '../migrate_create_executive_departure_snapshots.js';
...
describe('PLANNED_DDL — byte-pin', () => {
  it('starts with CREATE TABLE IF NOT EXISTS quantlab.executive_departure_snapshots', () => {
  ...
  it('executive_cluster_departure is UInt8 (boolean flag)', () => {
    assert.match(PLANNED_DDL, /executive_cluster_departure UInt8/);
  });
  it('flagged_sectors_json + per_ticker_json columns are String (variable-length payloads)', () => {
    assert.match(PLANNED_DDL, /flagged_sectors_json String/);
    assert.match(PLANNED_DDL, /per_ticker_json String/);
  });
  it('inputs_available_* are UInt32', () => {
  ...
  it('last_edgar_query_at + bd_since_last_query are Nullable', () => {
    assert.match(PLANNED_DDL, /last_edgar_query_at Nullable\(DateTime\)/);
    assert.match(PLANNED_DDL, /bd_since_last_query Nullable\(Int32\)/);
  });
...
describe('EXPECTED_COLUMNS — SPEC §6 alignment', () => {
  ...
  it('includes the aggregate flag + JSON payload columns', () => {
    for (const col of ['executive_cluster_departure', 'flagged_sectors_json', 'per_ticker_json']) {
      assert.ok((EXPECTED_COLUMNS as readonly string[]).includes(col), `missing ${col}`);
    }
  });
