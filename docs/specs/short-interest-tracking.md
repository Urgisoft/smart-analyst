...
**v1 implementation note — Path A4-β (autonomous resolution, HANDOFF s90, A4 commit):** FINRA's biweekly feed does NOT publish `shares_outstanding`; the column was an A1 SPEC adjustment. Rather than introducing a live yfinance `shares_outstanding` ingest at daemon-eval time (Path A4-α — adds a settlement-date asymmetry: yfinance publishes shares_outstanding as a single CURRENT value, not a settlement-date-aware historical series), v1 operates on **`shares_short` ROC directly** per Diether-Lee-Werner 2009 §3's dimensionally-invariant ROC formulation. The composite's `computeSIR(sharesShort, sharesOutstanding)` is called with `sharesOutstanding = 1`, so `computeSIR(sharesShort, 1) === sharesShort` — the SIR field in `ShortInterestInputs` carries raw `shares_short` values; ROC, D2C, `prior_high_base`, `short_ramp`, `short_capitulation`, and aggregate-z math are unchanged. The per-ticker baseline is the 2y trailing `shares_short` panel (not SIR); the aggregate baseline is the 2y trailing mean(`shares_short`) across SPY-500 constituents per settlement_date. ROC of `shares_short` equals ROC of SIR when `shares_outstanding` is constant; approximation error over the 3-month window (SPY-500 buyback/issuance rate < ~1-2%) is well below the 50% / 40% flag thresholds. A v2 enhancement re-integrating `shares_outstanding` via a CH-maintained snapshot table would be a separate ADR; composite version stays at `short_interest_v1` for the A4-β interpretation.
...
1. **Corporate-actions data source.** §4 / S-SI-6 split-adjustment: CH `corporate_actions` table OR yfinance live `actions` endpoint? Recommendation: yfinance for v1 (zero-infra), cache locally per ingest run. Migrate to a CH-managed table if yfinance reliability becomes an issue. **RESOLVED by Path A4-β (s90):** split-adjustment is moot in v1 because ROC is computed on `shares_short` directly (no SIR normalization). Splits scale `shares_short` and `shares_outstanding` identically, so even if SIR were used the ratio would be split-invariant up to vendor data quirks. The Path A4-β formulation eliminates the dependency on a corporate-actions feed for v1.

2. ~~**Per-ticker baseline window for `prior_high_base` qualifier on `short_capitulation`.**~~ **RESOLVED at A2 (s90 commit `7d80f74`):** 2y trailing per-ticker, matching the aggregate baseline + Diether-Lee-Werner §4. Path A4-β note: the baseline is on `shares_short` (not SIR) per §5.1's "v1 implementation note."

3. ~~**Aggregate weighting scheme.**~~ **RESOLVED at A2 (s90 commit `7d80f74`):** equal-weight primary, matching the academic literature default (Asquith-Pathak-Ritter unweighted, Diether-Lee-Werner aggregate unweighted). Zero free parameters. Path A4-β reinterpretation: equal-weight mean of `shares_short` across SPY-500-PIT constituents, z-scored against its own 2y baseline.
