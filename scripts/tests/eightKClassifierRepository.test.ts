/**
 * Tests for src/server/eight_k_classifier_repository.ts (Phase EK-A4).
 *
 * SPEC: docs/specs/event-driven-filings-processor.md §9.2 (T-EKR-1..T-EKR-Nplus6).
 *
 * Coverage:
 *   - exported constants (EVENT_WINDOW_DAYS, BASELINE_CALENDAR_DAYS,
 *     COMPOSITE_ITEM_CODES = HIGH_SIGNAL_ITEM_CODES re-export)
 *   - readLatestAcceptedAt (subquery-around-FINAL + 1970 sentinel + null)
 *   - readEventsForTickersInWindow (subquery-around-FINAL + item-code filter +
 ...
 *   - readSectorByTicker (G1-A3 / s94 #3 — PIT-DESC LIMIT 1 BY ticker
 *     + empty-tickers short-circuit + sub-industry coercion + asOf binding)
 *   - readInputsForCycle end-to-end (sector populated via gics_sector_map
 *     OR null on cold-start; aggregate sectors array still empty pending OQ-G2-1)
 ...
 *   - eightKClassifierSnapshotsTableExists + eightKEventsTableExists
 *   - runDaemonEightKClassifierEvaluation (orchestration + summary-line shape)
 *   - businessDaysBetween helper (parity with exec-departure / etf-flow)
...
function makeRepo() {
  ...
  const repo = new EightKClassifierRepository({ ch: fake as any });
  return { repo, fake };
}
...
describe('exported constants', () => {
  it('EVENT_WINDOW_DAYS mirrors composite ROLLING_WINDOW_DAYS (EK-3 = 90)', () => {
    assert.equal(EVENT_WINDOW_DAYS, 90);
  });
  it('BASELINE_CALENDAR_DAYS = 730 (EK-A4 2y baseline; matches exec-departure)', () => {
    assert.equal(BASELINE_CALENDAR_DAYS, 730);
  });
  it('COMPOSITE_ITEM_CODES is the HIGH_SIGNAL_ITEM_CODES re-export (load-bearing parity)', () => {
    // Reference equality — re-export, not text-copy. Drift catches at load-time.
    assert.equal(COMPOSITE_ITEM_CODES, HIGH_SIGNAL_ITEM_CODES);
    assert.deepEqual(
      [...COMPOSITE_ITEM_CODES],
...
describe('readEventsForTickersInWindow', () => {
  it('emits subquery-around-FINAL pattern with item-code + ticker filter', async () => {
    const { repo, fake } = makeRepo();
    fake.route(_ => true, []);
    await repo.readEventsForTickersInWindow(DATE, ['AAPL']);
    const sql = fake.queries[0].query;
    assert.match(sql, /FROM \(\s*SELECT[\s\S]+FROM \S+ FINAL[\s\S]+WHERE/);
    assert.match(sql, /item_code IN \({items:Array\(String\)}\)/);
  ...
  it('groups rows by ticker; parses accepted_at into Date', async () => {
    const { repo, fake } = makeRepo();
    fake.route(_ => true, [
      { ticker: 'AAPL', cik: '0000320193', accession: 'A1', item_code: '2.06', accepted_at: '2026-05-01 09:30:00' },
      { ticker: 'AAPL', cik: '0000320193', accession: 'A2', item_code: '4.02', accepted_at: '2026-05-10 11:00:00' },
      { ticker: 'MSFT', cik: '0000789019', accession: 'A3', item_code: '5.01', accepted_at: '2026-04-20 14:15:00' },
    ]);
    ...
    assert.equal(aaplFirst.itemCode, '2.06');
  ...
  it('drops rows with unparseable accepted_at', async () => {
    const { repo, fake } = makeRepo();
    fake.route(_ => true, [
      { ticker: 'AAPL', cik: '0000320193', accession: 'A1', item_code: '2.06', accepted_at: 'not-a-datetime' },
    ]);
    const out = await repo.readEventsForTickersInWindow(DATE, ['AAPL']);
    assert.equal(out.size, 0);
  });

  it('binds start + asOf + tickers + items as query params', async () => {
    const { repo, fake } = makeRepo();
    fake.route(_ => true, []);
    await repo.readEventsForTickersInWindow(DATE, ['AAPL']);
    ...
    assert.deepEqual(params.items, [...HIGH_SIGNAL_ITEM_CODES]);
  });

  it('respects custom windowDays override (e.g. 30d)', async () => {
    const { repo, fake } = makeRepo();
    fake.route(_ => true, []);
    await repo.readEventsForTickersInWindow(DATE, ['AAPL'], 30);
    const params = fake.queries[0].query_params ?? {};
...
// ───── readSectorByTicker (G1-A3 / s94 #3) ──────────────────────────
...
describe('readInputsForCycle', () => {
  it('composes inputs from EDGAR reads + CIK map + GICS sector map (G1-A3)', async () => {
    const { repo, fake } = makeRepo();
    ...
    // G1-A3: sector populated from gics_sector_map
...
function fixtureSnapshot(overrides: Partial<EightKClassifierSnapshot> = {}): EightKClassifierSnapshot {
  return {
    ...
    perTickerRows: [{
      ...
      daysSinceLatestEvent: 12,
      materialEventFlag: true,
      impairmentFlag: true,
      restatementFlag: false,
      auditorChangeFlag: true,
      delistingFlag: false,
      controlChangeFlag: false,
      materialAgreementFlag: false,
...
describe('writeSnapshot', () => {
  it('inserts a row with all 10 schema fields + per_ticker_json + flagged_sectors_json', async () => {
    ...
    assert.equal(row.eight_k_cluster_flag, 1);
    ...
    assert.equal(perTicker[0].materialEventFlag, true);
    assert.equal(perTicker[0].impairmentFlag, true);
    assert.equal(perTicker[0].auditorChangeFlag, true);
  ...
  it('maps version → composite_version column (load-bearing per S93-24)', async () => {
    const { repo, fake } = makeRepo();
    await repo.writeSnapshot(fixtureSnapshot());
    const row = fake.inserts[0].values[0];
    assert.equal(row.composite_version, 'eight_k_classifier_v1');
    assert.equal(row.version, undefined);
  });

  it('boolean→UInt8 for eight_k_cluster_flag', async () => {
    const { repo, fake } = makeRepo();
    await repo.writeSnapshot(fixtureSnapshot({ eightKClusterFlag: false }));
    assert.equal(fake.inserts[0].values[0].eight_k_cluster_flag, 0);
  });
...
describe('loadLatestSnapshot', () => {
  ...
  it('round-trips a populated snapshot row', async () => {
    ...
    const perTickerJson = JSON.stringify([{
      ...
      materialEventFlag: true, impairmentFlag: true, restatementFlag: false,
      auditorChangeFlag: true, delistingFlag: false, controlChangeFlag: false,
    ...
    fake.route(_ => true, [{
      ...
      composite_version: 'eight_k_classifier_v1',
    }]);
    const snap = await repo.loadLatestSnapshot();
    assert.ok(snap);
    const s = snap as EightKClassifierSnapshot;
    assert.equal(s.eightKClusterFlag, true);
  ...
  it('handles malformed per_ticker_json by degrading to empty array', async () => {
    ...
    const snap = await repo.loadLatestSnapshot();
    assert.ok(snap);
    assert.deepEqual((snap as EightKClassifierSnapshot).perTickerRows, []);
  });
  ...
  it('handles malformed flagged_sectors_json by degrading to empty array', async () => {
    ...
    const snap = await repo.loadLatestSnapshot();
    assert.ok(snap);
    assert.deepEqual((snap as EightKClassifierSnapshot).flaggedSectors, []);
  });
  ...
  it('decodes 1970-01-01 last_edgar_query_at as null', async () => {
    ...
    const snap = await repo.loadLatestSnapshot();
    assert.equal((snap as EightKClassifierSnapshot).lastEdgarQueryAt, null);
  });

  it('decodes eight_k_cluster_flag=0 to false', async () => {
    const { repo, fake } = makeRepo();
    fake.route(_ => true, [{
      snapshot_date: '2026-05-19',
      computed_at_ms: String(DATE.getTime()),
    ...
    const snap = await repo.loadLatestSnapshot();
    assert.equal((snap as EightKClassifierSnapshot).eightKClusterFlag, false);
  });
});

// ───── G3R-EK-* — max_aggregate_z persistence round-trip (OQ-G3-1) ─────
//
// SPEC docs/specs/gics-sector-baseline-computation.md §2 + HANDOFF S94-22.
// Mirror of executiveDepartureRepository.test.ts G3R-XD-* (byte-equal except
// for snapshot type + version constant). See that file for the rationale.

describe('G3R-EK — max_aggregate_z persistence (OQ-G3-1)', () => {
  it('G3R-EK-1: writeSnapshot stamps max_aggregate_z + max_aggregate_z_sector columns from the snapshot', async () => {
    const { repo, fake } = makeRepo();
    await repo.writeSnapshot(fixtureSnapshot({
      maxAggregateZ: 3.17,
      maxAggregateZSector: 'Healthcare',
    }));
    const row = fake.inserts[0].values[0];
    assert.equal(row.max_aggregate_z, 3.17);
    assert.equal(row.max_aggregate_z_sector, 'Healthcare');
    // Null pass-through.
    await repo.writeSnapshot(fixtureSnapshot({
      maxAggregateZ: null,
      maxAggregateZSector: null,
  ...
  it('G3R-EK-2: loadLatestSnapshot recovers max_aggregate_z + max_aggregate_z_sector from the CH row', async () => {
    ...
    populated.fake.route(_ => true, [{
      ...
      inputs_available_per_ticker: '0',
      composite_version: 'eight_k_classifier_v1',
      max_aggregate_z: 3.17,
      max_aggregate_z_sector: 'Healthcare',
    }]);
    const snap = await populated.repo.loadLatestSnapshot();
    assert.ok(snap);
    const s = snap as EightKClassifierSnapshot;
    assert.equal(s.maxAggregateZ, 3.17);
    assert.equal(s.maxAggregateZSector, 'Healthcare');
    ...
    assert.equal((snap2 as EightKClassifierSnapshot).maxAggregateZ, null);
    assert.equal((snap2 as EightKClassifierSnapshot).maxAggregateZSector, null);
  });
});

// ───── table-existence probes ──────────────────────────────────────

describe('eightKClassifierSnapshotsTableExists', () => {
  it('returns true when system.tables count > 0', async () => {
    const fake = new FakeClickHouse().route(_ => true, [{ n: 1 }]);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    assert.equal(await eightKClassifierSnapshotsTableExists(fake as any), true);
  });
  it('returns false when count = 0', async () => {
    const fake = new FakeClickHouse().route(_ => true, [{ n: 0 }]);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    assert.equal(await eightKClassifierSnapshotsTableExists(fake as any), false);
  });
...
describe('eightKEventsTableExists', () => {
  it('returns true when system.tables count > 0', async () => {
    const fake = new FakeClickHouse().route(_ => true, [{ n: 1 }]);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    assert.equal(await eightKEventsTableExists(fake as any), true);
  });
  it('returns false when count = 0', async () => {
    const fake = new FakeClickHouse().route(_ => true, [{ n: 0 }]);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    assert.equal(await eightKEventsTableExists(fake as any), false);
  });
  it('returns false when query throws', async () => {
    const fake = new FakeClickHouse();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
describe('runDaemonEightKClassifierEvaluation', () => {
  it('runs read → compute → write and returns a summary line', async () => {
    ...
    assert.equal(fake.inserts.length, 1);
    assert.equal(r.snapshot.eightKClusterFlag, false);
    assert.equal(r.snapshot.flaggedSectors.length, 0);
    assert.equal(r.snapshot.perTickerRows[0].materialEventFlag, true);
    assert.equal(r.snapshot.perTickerRows[0].impairmentFlag, true);
  });
  ...
  it('resolves universes from CH when not pre-passed', async () => {
    ...
    fake.route(q => q.includes('max(accepted_at)'), [{ last: null }]);
    fake.route(_ => true, []);
    const r = await runDaemonEightKClassifierEvaluation({ repo, asOf: DATE });
    assert.ok(r.snapshot);
    assert.match(r.summaryLine, /last_edgar=—/);
  });

  it('summary line renders staleness em-dash when no events ever ingested', async () => {
    const { repo, fake } = makeRepo();
    fake.route(q => q.includes('max(accepted_at)'), [{ last: null }]);
    fake.route(_ => true, []);
    const r = await runDaemonEightKClassifierEvaluation({
...
describe('CH grammar validation (EXPLAIN PLAN)', () => {
  it('readLatestAcceptedAt is EXPLAIN-clean', async (t) => {
    ...
    if (!verdict.ok && /Unknown table expression identifier.*eight_k_events/.test(verdict.failure?.error ?? '')) {
      return t.skip('quantlab.eight_k_events not yet created — first edgar ingest activates this check');
    }
  ...
  it('readSectorByTicker is EXPLAIN-clean (G1-A3 / s94 #3)', async (t) => {
    const { repo, fake } = makeRepo();
    fake.route(_ => true, []);
    await repo.readSectorByTicker(DATE, ['AAPL']);
    const verdict = await assertCHGrammar({ queries: fake.queries });
  ...
  it('loadLatestSnapshot is EXPLAIN-clean (skips when snapshots table absent)', async (t) => {
    ...
    const verdict = await assertCHGrammar({ queries: fake.queries });
    if (verdict.skipped) return t.skip('CH unreachable');
    if (!verdict.ok && /Unknown table expression identifier.*eight_k_classifier_snapshots/.test(verdict.failure?.error ?? '')) {
      return t.skip('quantlab.eight_k_classifier_snapshots not yet created — apply the EK-A3 migration to activate this check');
    }
    if (!verdict.ok) assert.fail(`EXPLAIN PLAN rejected:\n${verdict.failure?.error}\n---\n${verdict.failure?.query}`);
  });
});
