...
const EXTRA_HELP: HelpEntry[] = [
  ...
  { npm: 'etf:flow:ssga-spdr:fetch',           category: 'Data ingestion', what: 'Gap #9 v3.1 SSGA-SPDR navhist XLSX → canonical-CSV adapter. Fetches the public navhist-us-en-{ticker}.xlsx per SPDR ETF (13 in F-UNIVERSE: SPY/DIA + 11 sector XL*) and writes data/etf_flow_issuer_csv/ssga-spdr.csv for downstream etf:flow:issuer-csv:ingest. Schema-validated header anchors (R2 ticker + R4 columns) + skip-with-warn per-row + all-fail preserves last-good CSV. Supports --tickers / --lookback-days / --output-dir / --output-file. Run --source-label ssga-spdr on the downstream ingest.' },
  { npm: 'etf:flow:ssga-spdr:fetch:dry',       category: 'Data ingestion', what: 'Dry-run of `etf:flow:ssga-spdr:fetch` — fetch + parse + count without writing the CSV.' },
  { npm: 'etf:flow:ssga-spdr:refresh',         category: 'Data ingestion', what: 'Gap #9 v3.1 SSGA-SPDR end-to-end refresh — chains `etf:flow:ssga-spdr:fetch` (writes data/etf_flow_issuer_csv/ssga-spdr.csv) into `etf_flow_issuer_csv_ingest.py --source-label ssga-spdr --apply` (writes quantlab.etf_shares_outstanding_secondary). Plumbs the correct issuer source-label automatically so the secondary panel is tagged ssga-spdr (not the default issuer-csv). On fetch failure the ingest step is skipped via `&&`-chain; downstream CH ReplacingMergeTree(ingested_at) preserves the last-good row set.' },
