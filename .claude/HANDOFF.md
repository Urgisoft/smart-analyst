...
1. **Quartz docs site + status frontmatter + auto-generated dashboard** (operator-queued s95 #4, architecture confirmed) — pre-scoped, ready to execute. **5 commits, ~600 LOC, ~6 tests.** Operator says "do the Quartz slice" / "start docs visualization" to kick off. Pre-locked architecture (S95-20..S95-23 below):
   ...
   - **Quartz config:** v4, source = `docs/`, output = `docs/.quartz-site/` (gitignored), dark theme + dense Bloomberg-style layout, plugins = ObsidianFlavoredMarkdown (includes Mermaid + callouts + wikilinks) + Latex + SyntaxHighlighting. npm scripts: `docs:build` (build), `docs:serve` (build --serve → <http://localhost:8080> with HMR), and `dev:all` (concurrently runs `npm:dev` on :3000 + `npm:docs:serve` on :8080 in one terminal with color-coded logs). Bare `npm run dev` stays app-only by design (skip Quartz noise when not touching docs); `dev:all` is opt-in for sessions that need both.
