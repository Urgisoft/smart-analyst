/**
 * Tests for scripts/_probe_phase_b_sector_rot_v1_inputs.ts — Cycle 25
...
} from '../_probe_phase_b_sector_rot_v1_inputs.js';

describe('Probe SPEC constants — anti-drift pins', () => {
  it('REQUIRED_WINDOW_START = "2013-01-03" per SPEC §S-PBSR1-5', () => {
  ...
  it('REQUIRED_EARLIEST_DATE_MAX ≤ 2014-01-01 (full-strength baseline)', () => {
    // Snapshots earliest_date must precede 2014-01-01 for the trailing-1y
    // (252-day) baseline to be fully populated at SPEC IS_START (= 2013-01-03).
...
describe('Probe constants — alignment with vol_struct_v1 probe (cross-arc parity)', () => {
  it('REQUIRED_WINDOW_START matches vol_struct_v1 probe (cross-composite parity)', async () => {
    // SPEC §S-PBSR1-5: match vol_struct_v1's window for cross-composite
  ...
  it('thresholds match vol_struct_v1 (anti-drift across probes)', async () => {
    const volStructProbe = await import('../_probe_phase_b_vol_struct_v1_inputs.js');
    assert.equal(SPARSE_ROW_THRESHOLD, volStructProbe.SPARSE_ROW_THRESHOLD);
    assert.equal(REQUIRED_EARLIEST_DATE_MAX, volStructProbe.REQUIRED_EARLIEST_DATE_MAX);
    assert.equal(EMPTY_OR_FORWARD_ONLY_ROW_THRESHOLD, volStructProbe.EMPTY_OR_FORWARD_ONLY_ROW_THRESHOLD);
    assert.equal(FORWARD_ONLY_EARLIEST_DATE_MIN, volStructProbe.FORWARD_ONLY_EARLIEST_DATE_MIN);
  });
});

describe('Probe column-name convention pin', () => {
  it('probe source references defensive_cyclical_spread_z (NOT curve_steepness_z)', async () => {
    const src = await import('node:fs').then(fs =>
      fs.readFileSync('scripts/_probe_phase_b_sector_rot_v1_inputs.ts', 'utf-8'),
    );
    assert.match(src, /defensive_cyclical_spread_z/,
      'probe must reference defensive_cyclical_spread_z (the score column)');
    // Belt-and-suspenders: ensure no leftover curve_steepness_z from
    // copy-paste of vol_struct_v1 probe.
    assert.ok(!src.match(/curve_steepness_z/i),
      'probe must NOT reference curve_steepness_z (vol_struct_v1 column)');
  });
  it('probe source references sector_rotation_snapshots (NOT vol_structure_snapshots)', async () => {
    const src = await import('node:fs').then(fs =>
      fs.readFileSync('scripts/_probe_phase_b_sector_rot_v1_inputs.ts', 'utf-8'),
    );
