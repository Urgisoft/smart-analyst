/**
 * Short-interest snapshots table creation (Phase A3).
 *
 * SPEC: docs/specs/short-interest-tracking.md §6 (schema) + §3 (component diagram).
 *
 * Why CREATE TABLE IF NOT EXISTS, not s82-style CREATE-NEW + RENAME:
 *   Brand-new table; nothing to preserve. Same s84/s85/s86/s87/s88 simple-
 ...
 *   - Float32 (not Float64) — matches cross_asset_snapshots idiom. SIR values
 *     are 0-1 range, z-scores typically ±5; Float32 precision is sufficient
 *     and halves storage footprint at the per-ticker JSON payload scale.
 ...
 *   - `composite_version` (not `version`) — matches cross-asset column name.
 ...
 *   - `index_granularity = 8192` (cross-asset default), not the SPEC's 1024.
 *     The per-ticker JSON payload is variable-length but typically small
 *     enough that 8192-granularity index marks are not the bottleneck.
 *
 * Usage:
 *   npm run migrate:create-short-interest-snapshots             # dry-run
 *   npm run migrate:create-short-interest-snapshots:apply       # executes CREATE
 */
...
export const help: HelpEntry[] = [
  {
    npm: 'migrate:create-short-interest-snapshots',
    category: 'Data quality',
    what:
      'Dry-run: show planned CREATE TABLE for quantlab.short_interest_snapshots ' +
      '(short-interest-tracking SPEC §6). No DDL executed.',
  },
  {
    npm: 'migrate:create-short-interest-snapshots:apply',
    category: 'Data quality',
    what:
      'APPLY the short-interest snapshots CREATE TABLE migration. ' +
      'Destructive — operator-authorized.',
  },
];

export const DATABASE = 'quantlab';
export const TABLE = 'short_interest_snapshots';
...
(
  ...
  aggregate_sir Nullable(Float32),
  aggregate_z Nullable(Float32),
  aggregate_baseline_size UInt32,
  sentiment_short_extreme UInt8,
...
export const EXPECTED_COLUMNS = [
  ...
  'last_finra_publication', 'bd_since_publication',
  'aggregate_sir', 'aggregate_z',
  'aggregate_baseline_size', 'sentiment_short_extreme',
  'per_ticker_json',
