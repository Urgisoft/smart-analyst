...
7. `sentiment_extreme` (NEW, **dual-source per §2.1**) — fires if EITHER:
   - CBOE ^CPC 5d MA ≥ **1.15** (extreme fear) OR ≤ **0.77** (extreme complacency) — both tails of the contrarian sentiment indicator (Whaley 2009 §3, CBOE methodology). The complacency floor was recalibrated from 0.65 in session 78 (see §2.3 footnote a). The fear ceiling 1.15 is unchanged but empirically validated in session 78 at p95 of the 5d-MA distribution (5.46% corpus-wide fire rate; per-regime stability [4.75%, 6.73%]). OR
...
**Footnote (a) — `PUT_CALL_COMPLACENCY_LOW` retune (session 78).** The session-39 Tier 0 pick `≤ 0.65` was preserved through phase1_v3 ship because `macro_indicators_cboe` was empty and the threshold couldn't be stress-tested. With the CBOE free historical archive now ingested (2003-10-17 → 2019-10-04, 4,014 5d-MA rows), quantile matching against the empirical 5d-MA distribution shows the 0.65 floor fired only **0.17%** (7 of 4,014 days) — effectively dormant, calibrated to the bottom-1.7% tail rather than the design ~5%. The session-78 retune to `≤ 0.77` is the smallest 2-decimal floor at or above the corpus p05 (0.7620, inside-toward-zero round), so firings sit in the bottom-5% complacency tail (same methodology as the session-40 VIX/VIX3M retune). Post-tune corpus fire rate is 6.23%; per-regime p05 range [0.711, 0.826] brackets 0.77 across pre-GFC / GFC / post-GFC / 2015-2019-calm, with the calm regime running tighter than corpus (consistent with Whaley §3's "complacency floors relative to long-run norms, not the calm-cycle's own internal norm"). Operational status: gate (a) was closed in session 79 — a fresh `macro:backfill:v3` run joined CBOE into `macro_regimes` for the 2003-2019 archive window (see the ADR-038 amendment v3 and the `ADR_038_BASELINE` re-pin in session 79), so `sentiment_extreme` now fires off this constant for the 2008-2019 portion of the phase1_v3 corpus. Gate (b) — CBOE 2019-present via the DataShop paid subscription — remains open; until that gate closes, post-2019 days fall through to the VIX/VIX3M arm alone, and a second backfill will extend live coverage once DataShop ingest is restored. Diagnostic: `scripts/_diagnose_put_call_thresholds.ts`.
...
4. **Extend `classifyMacroRegime`** in `src/server/macro_regime.ts`:
   ...
   - Add named threshold constants (`YIELD_CURVE_PERSISTENCE_DAYS=3`, `CREDIT_STRESS_20D_RETURN_FLOOR=-0.03`, `RISK_OFF_SPREAD_FLOOR_PP=-0.10`, `PUT_CALL_FEAR_HIGH=1.15`, `PUT_CALL_COMPLACENCY_LOW=0.77` — initial pick was 0.65; recalibrated to 0.77 in session 78 per §2.3 footnote (a). `VIX_TERM_COMPLACENCY_FLOOR=0.80` — initial pick was 0.85; recalibrated to 0.80 in session 40 per §2.3 footnote).
